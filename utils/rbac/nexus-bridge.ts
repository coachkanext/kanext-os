/**
 * Nexus RBAC Bridge — Maps constitutional role definitions to Nexus capability levels.
 *
 * Nexus uses a simplified 9-level (R1-R9) capability model.
 * This bridge derives the Nexus level from the constitutional Authority + Decision
 * dimensions, and preserves all existing capability matrices and action mappings.
 */

import type { Mode } from '@/types';
import type { RBACLevel, NexusCapability } from '@/types/nexus-v2';
import type { UniversalRoleDefinition } from './constitution';
import { Authority, Decision } from './constitution';

// =============================================================================
// CONSTITUTIONAL → NEXUS LEVEL MAPPING
// =============================================================================

/**
 * Derives a Nexus R1-R9 level from a UniversalRoleDefinition.
 * Uses authority + decision dimensions as the primary signal.
 */
export function mapDefinitionToNexusLevel(def: UniversalRoleDefinition): RBACLevel {
  // Institutional authority (A5) → full Nexus access
  if (def.authority >= Authority.Institutional) return 'R1';

  // Domain governance (A4) → program-level Nexus
  if (def.authority >= Authority.DomainGov) return 'R2';

  // Program governance (A3) → ministry/director level
  if (def.authority >= Authority.ProgramGov) return 'R3';

  // Execution (A2) with tactical+ decision → assistant/professional
  if (def.authority >= Authority.Execution) {
    if (def.isExternal) return 'R5';
    return 'R4';
  }

  // Personal (A1) — self-only actors
  if (def.authority >= Authority.Personal) {
    if (def.isSelfOnly) return 'R6';
    if (def.decision >= Decision.Self) return 'R7';
    return 'R8';
  }

  // Observer (A0) — external or public
  if (def.isExternal) return 'R5';
  return 'R9';
}
...
// =============================================================================
// ROLE MAPPING (backward-compat bridge from existing app roles)
// =============================================================================
