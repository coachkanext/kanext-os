...
/* ------------------------------------------------------------------ */
/* Sort-key → SQL expression mapping                                   */
/* ------------------------------------------------------------------ */
...
const SORT_MAP: Record<string, string> = {
  ...
  // Shooting (derived)
  ts_pct:
    "CASE WHEN (pst.fga + 0.44 * pst.fta) > 0 THEN pst.pts::float / (2 * (pst.fga + 0.44 * pst.fta)) END",
  efg_pct:
    "CASE WHEN pst.fga > 0 THEN (pst.fgm + 0.5 * pst.three_pm)::float / pst.fga END",
  two_pct:
    "CASE WHEN (pst.fga - pst.three_pa) > 0 THEN (pst.fgm - pst.three_pm)::float / (pst.fga - pst.three_pa) END",
  three_par:
    "CASE WHEN pst.fga > 0 THEN pst.three_pa::float / pst.fga END",
  // Advanced (derived)
  ast_pct:
    "CASE WHEN pst.min_total > 0 AND tss.min > 0 AND (5.0 * pst.min_total * tss.fgm / tss.min - pst.fgm) > 0 THEN 100.0 * pst.ast / (5.0 * pst.min_total * tss.fgm / tss.min - pst.fgm) END",
  usg_pct:
    "CASE WHEN pst.min_total > 0 AND (tss.fga + 0.44 * tss.fta + tss.tov) > 0 THEN (pst.fga + 0.44 * pst.fta + pst.tov)::float * (tss.min / 5.0) / (pst.min_total * (tss.fga + 0.44 * tss.fta + tss.tov)) END",
  tov_pct:
    "CASE WHEN (pst.fga + 0.44 * pst.fta + pst.tov) > 0 THEN pst.tov::float / (pst.fga + 0.44 * pst.fta + pst.tov) END",
  stl_pct: "NULL",
  blk_pct: "NULL",
  reb_pct: "NULL",
  per: "NULL",
  // Per-40
  pts_40:
    "CASE WHEN pst.min_total > 0 THEN pst.pts::float / pst.min_total * 40 END",
  reb_40:
    "CASE WHEN pst.min_total > 0 THEN pst.trb::float / pst.min_total * 40 END",
  ast_40:
    "CASE WHEN pst.min_total > 0 THEN pst.ast::float / pst.min_total * 40 END",
  stl_40:
    "CASE WHEN pst.min_total > 0 THEN pst.stl::float / pst.min_total * 40 END",
  blk_40:
    "CASE WHEN pst.min_total > 0 THEN pst.blk::float / pst.min_total * 40 END",
  to_40:
    "CASE WHEN pst.min_total > 0 THEN pst.tov::float / pst.min_total * 40 END",
  // Scoring
  pct_2pt:
    "CASE WHEN pst.pts > 0 THEN ((pst.fgm - pst.three_pm) * 2)::float / pst.pts END",
  pct_3pt:
    "CASE WHEN pst.pts > 0 THEN (pst.three_pm * 3)::float / pst.pts END",
  pct_ft:
    "CASE WHEN pst.pts > 0 THEN pst.ftm::float / pst.pts END",
  ft_rate:
    "CASE WHEN pst.fga > 0 THEN pst.fta::float / pst.fga END",
  pts_fga:
    "CASE WHEN pst.fga > 0 THEN pst.pts::float / pst.fga END",
  ppp:
    "CASE WHEN (pst.fga + 0.44 * pst.fta + pst.tov) > 0 THEN pst.pts::float / (pst.fga + 0.44 * pst.fta + pst.tov) END",
};
