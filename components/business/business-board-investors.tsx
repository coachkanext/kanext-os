    ...
    <View>
      <BizCard>
        ...
        <ThemedText style={[s.sectionSubtitle, { color: BP.ash }]}>
          ...
          {BOARD_SEATS.filter((s) => s.votingRights).length} voting
        </ThemedText>
        ...
        {visibleDocs.map((doc, idx) => (
          <Pressable
            ...
            style={({ pressed }) => [
              s.docRow,
              { borderColor: BP.graphite, opacity: pressed ? 0.8 : 1 },
              idx < visibleDocs.length - 1 && s.docRowSpaced,
            ]}
            onPress={() => Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light)}
          >
            {/* Icon + title */}
        ...
        {RESOLUTIONS.map((resolution, idx) => (
          ...
          >
            ...
            <View style={s.resolutionHeader}>
              ...
              <BizStatusChip
                label={resolution.outcome.toUpperCase()}
                variant={statusVariant(resolution.outcome)}
              />
            </View>
