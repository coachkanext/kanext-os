/**
 * Phone Side Panel — Personal Mode.
 * Recents · Contacts · Voicemail + Dipson/Settings
 */
...
const MAIN_NAV: NavItem[] = [
  { icon: 'phone.fill',           label: 'Calls',      route: '/(tabs)/(main)/phone/calls' },
  { icon: 'person.crop.circle',   label: 'Contacts',   route: '/(tabs)/(main)/phone/contacts' },
...
const BOTTOM_ITEMS: NavItem[] = [
  ...
  { icon: 'gearshape',            label: 'Settings',   route: '/(tabs)/(main)/phone/settings' },
];
...
export function PhonePanel() {
  ...
  const go = useCallback((item: NavItem) => {
    ...
    if (item.isDipson) {
      closeSidePanel();
      setTimeout(() => openDipsonSheet('Phone'), 300);
      return;
    }
    closeSidePanel();
    setTimeout(() => {
