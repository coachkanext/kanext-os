...
export function BusinessKplayPanel() {
  const C = useColors();
  const s = useMemo(() => makeStyles(C), [C]);
  const router = useRouter();
  const [role, , roleCycles] = useDemoRole('business:kaystudios');
    ...
    <View style={s.root}>
      <View style={s.header}>
        <View style={[s.avatar, { backgroundColor: '#1E3A28' }]}>
          <Text style={[s.avatarText, { color: '#F0E8DC' }]}>KN</Text>
        </View>
