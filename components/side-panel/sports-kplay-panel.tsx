/**
 * Sports KPlay Side Panel.
 * All roles: Browse · Library + divider + Dipson · Settings · Help
 * Head Coach can long-press content in Browse to assign as Required.
 */
...
export function SportsKplayPanel() {
  const C = useColors();
  const s = useMemo(() => makeStyles(C), [C]);
  const router = useRouter();
  const [role, , roleCycles] = useDemoRole('sports:kaystudios');
