/**
 * Education Social Side Panel — X/Twitter-style, role-aware.
 * President: Feed · Explore · Drafts · Scheduled · Posting Policy
 *            Bottom: Dipson · Settings · Help
 * Student:   Feed · Explore
 *            Bottom: Dipson · Settings · Help
 */
...
export function EducationSocialPanel() {
  const C = useColors();
  const s = useMemo(() => makeStyles(C), [C]);
  const router = useRouter();
  const [role, , roleCycles] = useDemoRole('education:social');
    ...
    <View style={s.root}>
      ...
      <View style={s.header}>
        <View style={[s.avatar, { backgroundColor: '#3A2E1A' }]}>
          <Text style={s.avatarInitial}>LU</Text>
        </View>
