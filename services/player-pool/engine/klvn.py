...
# Canonical Legend ordering — mechanical spacing.
# NCAA D1 High Major = 1.000 (reference). Lower levels get discounted.
# "ncaa_d1" is NOT allowed — must be remapped to HM/MM/LM before KLVN.
...
def get_lambda(level_key: str) -> float:
    ...
    lam = LEVEL_LAMBDA.get(level_key)
    if lam is None:
        raise ValueError(
            f"KLVN HARD ERROR: level_key '{level_key}' not in LEVEL_LAMBDA. "
            f"Valid keys: {sorted(LEVEL_LAMBDA.keys())}. "
            f"If ncaa_d1, it must be remapped to ncaa_d1_high_major/mid_major/low_major first."
        )
    return lam


def get_poss_prior(level_key: str) -> float:
    val = POSSESSION_PRIOR.get(level_key)
    if val is None:
        raise ValueError(f"KLVN HARD ERROR: level_key '{level_key}' not in POSSESSION_PRIOR.")
    return val
...
def shrink(observed: float, n_events: int, level_key: str, baseline: float | None = None) -> float:
    ...
    alpha = SHRINKAGE_ALPHA.get(level_key)
    if alpha is None:
        raise ValueError(f"KLVN HARD ERROR: level_key '{level_key}' not in SHRINKAGE_ALPHA.")
...
def compute_confidence(games: int, level_key: str) -> float:
    ...
    g_target = GAMES_TARGET.get(level_key)
    if g_target is None:
        raise ValueError(f"KLVN HARD ERROR: level_key '{level_key}' not in GAMES_TARGET.")
