...
## Purpose
This protocol defines how the Player Evaluation Pipeline operates at V1 data tier (box score + advanced composites). It supplements the standard pipeline by providing V1-specific rules for handling the 31 of 47 traits that cannot be scored from box score data.

At V1, the full 47-trait OPF math cannot produce accurate absolute KR values because 60% of traits are NULL. This protocol combines production-based anchoring (Phase 3) with trait-level math (Phase 6) to produce honest, separated KR estimates.

As data tier improves (V1+ → V2 → V3), Phase 6 earns more authority and Phase 3 earns less. At V3, Phase 6 runs on real data and Phase 3 becomes a validation check only.
...
- Advanced composites (derived): TS%, eFG%, AST/TO, Usage%, ORTG, DRTG, BPM (Off/Def/Overall), PER

Trait coverage at V1: 16 traits solidly scorable, 5 weak proxies, 31 NULL.

## THE FIVE STEPS

### STEP 1: COACH CONTEXT

Set the program, level, governing body, division, major class (if D1), offensive system, and defensive system. Unchanged from the standard pipeline — Coach Context is Step 0 regardless of data tier. Coach Context binds: KLVN level key, KR legend selection, position trait weighting (OPF), system demand profiles.

### STEP 2: PHASE 3 — PRODUCTION ANCHOR

Map the player's production profile against the KR legend tiers at their level to establish an anchor range.

Inputs: Per-game stats, advanced composites, minutes/games, role on team (primary/secondary/role player/bench), awards, team success context.

Process: Read the legend tier descriptions at the player's level. Find the tier whose description matches the player's actual production and role. The description includes role language, statistical anchors (BPR ranges, MPG ranges), and outcome markers.

Output: A KR range (e.g., 88–91) representing where this player's production pattern fits in the legend.

Rules:
- Phase 3 uses the FULL production picture, not just scoring
- Phase 3 does NOT project — evaluates current season production only
- Phase 3 does NOT use recruiting rankings, draft projections, or future potential
- Tier 3 inputs (scouting, coach quotes, pedigree) can CONFIRM a range but cannot push it higher than production supports

### STEP 3: PHASE 6 — OPF MATH WITH BOUNDING + PROXY CONFIDENCE

Score traits from box score data. Bound NULL traits using composite metrics. Apply proxy confidence weights. Run OPF math.

**3A: Score SCORABLE traits** using standard box-score proxy formulas from the Trait Library.

TRUE traits (scored directly): Free Throw, Height, Length (if available).

PROXY traits (scored from proxy formulas): 3PT Spot-Up, Rim Pressure, Foul Draw, Advantage Creation, Passing Vision, Passing Execution, Ball Security, Deflections, Steal Timing, Foul Discipline, Rim Protection/Shot Blocking, Defensive Rebounding, Offensive Rebounding, Strength, Vertical Pop, Motor, Endurance.

**3B: Compute composite bands** for bounding NULL traits.

Classify each advanced composite into a band (90/80/70/60/50) using level-specific thresholds.
...
Other levels scale via KLVN lambda. Thresholds are v0 estimates — need calibration from actual stat distributions.

**3C: Compute NULL trait bounds by cluster.**

Offensive NULLs:
- Standard (USG% ≥ 20%): Off_Bound = 0.50×Band(Off BPM) + 0.30×Band(USG%) + 0.20×Band(TS%)
- Low-usage (USG% < 20%): Off_Bound = 0.30×Band(Off BPM) + 0.15×Band(USG%) + 0.55×Band(TS%)
...
Defensive NULLs: Def_Bound = 0.50×Band(Def BPM) + 0.30×Band(STL%) + 0.20×Band(BLK%)

IQ NULLs: IQ_Bound = 0.40×Band(Overall BPM) + 0.35×Band(AST/TO) + 0.25×Band(inverse TOV%)

Tools NULLs (Speed, Lateral Quickness): Bounded by the average of the player's own scored Tools traits (Height, Length, Strength, Vertical Pop) ±3, capped by position ceilings:
- Center: Speed cap 78, Lateral Quickness cap 76
- PF: Speed cap 80, Lateral Quickness cap 78
- SF/SG/PG: no caps

Rebounding NULLs (Box Outs, Rebound Range, Hands/Secure): Bounded by average of Band(DRB%) and Band(ORB%).

Each NULL trait estimate = midpoint of bounded range.

**3D: Apply proxy confidence weights.**

Each PROXY trait is blended with its cluster's composite bound:

Effective_Score = (Confidence_Weight × PROXY_Score) + ((1 − Confidence_Weight) × Composite_Bound_Midpoint)

TRUE traits: Confidence = 1.0 (no blending). NULL traits: Confidence = 0.0 (full bound applies).

Proxy Confidence Weights:

| Trait | Confidence | Notes |
|---|---|---|
| 3PT Spot-Up | 0.90 | 3P% directly measures shooting |
| Free Throw | 1.00 | TRUE score |
| Rim Pressure | 0.70 | FTA + 2PA decent but includes non-rim 2PA |
...
| Deflections | 0.45 | STL ≠ deflections but correlated |
...
| Strength | 0.55 | Weight is fact but strength ≠ weight |
...
**3E: Run OPF math** across all 47 traits (scored, bounded, blended) to compute OKR, DKR, TKR, IQKR.

Composite KR = (OKR × OPF_Off%) + (DKR × OPF_Def%) + (TKR × OPF_Tools%) + (IQKR × OPF_IQ%)
...
- If Phase 6 raw is near or above Phase 3 midpoint: scored traits confirm the production anchor. Final KR at or above midpoint.
- If Phase 6 raw is below Phase 3 low: scored traits reveal weaknesses. Final KR at or below midpoint, adjusted by downward signal strength.

Adjustment method: Starting from Phase 3 midpoint, apply directional pressure from Phase 6. Each OPF component pushes UP or DOWN relative to expectations for a player at the Phase 3 midpoint, weighted by OPF percentage. Strong confirmed weaknesses push down harder. Strong confirmed strengths push up. Total adjustment bounded: maximum −10 from Phase 3 low, maximum +10 from Phase 3 high.

### STEP 5: FINAL KR

The adjusted number is the V1 KR.
...
Output includes: Final KR, KR Range, Confidence %, Phase 3 anchor (transparency), Phase 6 raw (transparency), key trait strengths/weaknesses, legend tier label at the player's home level AND at other relevant levels (Level Tier Map).

## KR IS UNIVERSAL — CRITICAL RULE

The KR output is a single universal number. It does not change based on level context. DO NOT multiply KR by KLVN lambda to create "level-equivalent" numbers. There is no "HM-equivalent KR."

KLVN lambda normalizes INPUTS (production stats) during trait scoring. It does NOT convert KR OUTPUTS.

When presenting a player's KR, show one number with multiple legend reads:
- "KR: 85"
- At MM: Solid Starter
- At HM: Reliable Bench / Rotation Contributor
- At LM: High-Impact Starter

This is the Level Tier Map from the Development Engine. One player, one KR, multiple interpretations.
...
Tested on 56 players across 7 teams:
- Kansas 2025-26 (Self, Post-Centric + Containment Man, D1 HM): 8 players, 93-75 spread
- Florida 2025-26 (Golden, Post-Centric/DD + Containment Man, D1 HM): 8 players, 93-80 spread
- Kentucky 2025-26 (Pope, Pace & Space + Containment Man, D1 HM): 8 players, 91-80 spread
- Michigan 2025-26 (May, Motion/R&R + Switch/Containment, D1 HM): 8 players, 96-81 spread
- Arizona 2025-26 (Lloyd, Motion/R&R + Containment Man, D1 HM): 8 players, 94-81 spread
- Arkansas 2025-26 (Calipari, Pace & Space/DD + Containment Man, D1 HM): 8 players, 96-81 spread
- Memphis 2025-26 (Hardaway, Dribble Drive/P&S + Containment Man, D1 MM): 8 players, 85-77 spread

All rosters showed correct hierarchy, clean tier breaks, and legend labels matching observed reality. KR stacks correlate with team success across all 7 teams.

Known calibration gaps:
- Composite band thresholds (v0) need calibration from actual stat distributions at each level
- Proxy confidence weights are role-agnostic — may need role-conditional adjustment for primary ball-handlers
- Tested at D1 HM and D1 MM only — lower levels need testing
- Phase 3 ±10 window is provisional

## Governance

- V1 Protocol supplements the standard Player Evaluation Pipeline
- When data tier upgrades, V1 output is superseded by the standard pipeline
- All V1 outputs flagged as V1_EVALUATION with confidence cap
- Phase 3 anchor ranges logged alongside Phase 6 raw for audit trail
- Composite band thresholds and proxy confidence weights subject to versioned calibration updates