...
export const SYSTEM_OWNER_MEMBERSHIP_IDS: Set<string> = new Set([
...
/**
 * AnyRoleLens is re-declared here to avoid circular imports.
 * The canonical type lives in the dispatcher; this is a structural match.
 */
type SystemOwnerLens =
  | { mode: 'sports'; lens: 'R0' }
  | { mode: 'church'; lens: 'C0' }
  | { mode: 'business'; lens: 'B0' }
  | { mode: 'education'; lens: 'E0' }
  | { mode: 'competition'; lens: 'CO0' };

/**
 * Returns the highest-privilege lens for a given mode.
 * Used by `resolveRoleLens()` to bypass per-mode membership lookups.
 */
export function getSystemOwnerLensForMode(mode: Mode): SystemOwnerLens {
  switch (mode) {
    case 'sports':
      return { mode, lens: 'R0' };
    case 'church':
