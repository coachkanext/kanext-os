/**
 * Community Social Side Panel — role-aware.
 * Pastor: Feed · BROWSE (Explore) · MANAGE (Drafts, Scheduled, Posting Policy)
 * Member: Feed · BROWSE (Explore)
 */
...
const BROWSE_ITEMS: NavItem[] = [
  { icon: 'safari.fill', label: 'Explore', route: '/(tabs)/(main)/social/explore' },
];

const MANAGE_ITEMS: NavItem[] = [
  { icon: 'doc.fill',        label: 'Drafts',          route: '/(tabs)/(main)/social/drafts'          },
  { icon: 'clock.fill',      label: 'Scheduled',       route: '/(tabs)/(main)/social/scheduled'       },
  { icon: 'lock.shield.fill',label: 'Posting Policy',  route: '/(tabs)/(main)/social/posting-policy'  },
];

// ── Component ─────────────────────────────────────────────────────────────────

export function CommunitySocialPanel() {
  const C = useColors();
  const s = useMemo(() => makeStyles(C), [C]);
  const router = useRouter();
  const [role, , roleCycles] = useDemoRole('community:social');
  ...
  return (
    <View style={[s.root, { backgroundColor: C.surface }]}>
      ...
      <Pressable
        ...
        onPress={() => go('/(tabs)/(main)/social/index')}
      >
