/**
 * Business Social Side Panel — X/Twitter-style, role-aware.
 * CEO:      Feed · Explore · Drafts · Scheduled · Posting Policy
...
export function BusinessSocialPanel() {
  const C = useColors();
  const s = useMemo(() => makeStyles(C), [C]);
  const router = useRouter();
  const [role, , roleCycles] = useDemoRole('business:social');
  ...
  return (
    <View style={s.root}>
      ...
      {/* ── Identity header ── */}
      <View style={s.header}>
        <View style={[s.avatar, { backgroundColor: '#1E3A28' }]}>
